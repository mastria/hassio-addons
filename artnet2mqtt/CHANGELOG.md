...
### Fixed
- MQTT connection refused (`[Errno 111]`) on startup no longer crashes the process.
  The addon now retries the initial connection with exponential backoff (2 s → 60 s max)
  until the broker (`core-mosquitto`) is ready, instead of exiting and being restarted
  in a loop by s6. ArtNet reception now starts reliably after the broker is reachable.
